...
Item {
  ...
  property string calculationQuery: ""
  property int calculationRevision: 0
  property bool calculationPending: false
  ...
  function looksLikeCalculation(query) {
    var input = String(query || "").trim()
    if (!input || input.length > 4096 || !/\d/.test(input)) return false

    var startsWithNumber = /^\s*[+-]?\s*(?:\d|\.\d|\([\s(+-]*(?:\d|\.\d))/.test(input)
    var hasOperator = /[+\-*\/%^]/.test(input)
    var hasCurrencyConversion = /\b[A-Za-z]{3}\b.*\b(?:to|in)\b.*\b[A-Za-z]{3}\b/i.test(input)
    var isIncomplete = /[+\-*\/%^(]\s*$/.test(input)
    return startsWithNumber && (hasOperator || hasCurrencyConversion) && !isIncomplete
  }

  function requestCalculation(query) {
    root.calculationRevision += 1
    root.calculationResult = ""
    root.calculationQuery = String(query || "").trim()
    root.calculationPending = root.looksLikeCalculation(root.calculationQuery)
    calculationTimer.stop()
    if (root.calculationPending) calculationTimer.restart()
  }

  function runCalculation() {
    if (!root.calculationPending || !root.looksLikeCalculation(root.calculationQuery)) return
    if (calculationProc.running) {
      calculationTimer.restart()
      return
    }

    root.calculationPending = false
    calculationProc.revision = root.calculationRevision
    calculationProc.query = root.calculationQuery
    calculationProc.collected = ""
    calculationProc.command = ["qalc", "-t", "-m", "1500", root.calculationQuery]
    calculationProc.running = true
  }

  function copyCalculation(result) {
    calculationCopyProc.command = ["wl-copy", "--", String(result || "")]
    calculationCopyProc.running = true
    root.opened = false
    root.filterText = ""
  }
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      if (root.calculationResult && root.calculationQuery === query) {
        currentRows.push({
          itemId: "calculation.result",
          kind: "calculation",
          icon: "󰃬",
          iconFont: "",
          appIcon: "",
          appId: "",
          label: root.calculationResult,
          target: "",
  ...
  function setFilter(nextFilter) {
    ...
    root.requestCalculation(root.filterText)
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "menu" || row.kind === "link") {
      ...
      root.copyCalculation(root.calculationResult)
    } else {
  ...
  Process {
    ...
    stdout: SplitParser {
      onRead: function(data) { calculationProc.collected += data + "\n" }
    }
    onExited: function(exitCode, exitStatus) {
      if (exitCode === 0 && exitStatus === 0
          && calculationProc.revision === root.calculationRevision
          && calculationProc.query === root.filterText.trim()) {
        var result = calculationProc.collected.trim()
        if (result) root.calculationResult = result
        root.rebuildDisplay()
      }
      if (root.calculationPending) calculationTimer.restart()
    }
  ...
  Process { id: calculationCopyProc }
